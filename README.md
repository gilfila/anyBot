...
anyBot 0.2.16+ supports one-click in-app updates via `electron-updater`. The update feed must be a publicly accessible HTTPS endpoint hosting:

- Windows installer: `anyBot Setup X.Y.Z.exe`
- Metadata file: `latest.yml` (generated by electron-builder)
- Optional: blockmap files for delta updates

**Update feed configuration:**

Set the `ANYBOT_UPDATE_FEED_URL` environment variable to enable automatic updates:

```powershell
$env:ANYBOT_UPDATE_FEED_URL = "https://releases.example.com/anybot/desktop"
```

Or configure it at build time by modifying `desktop/main.cjs`.

**Security considerations:**

- The source repository remains private; only compiled binaries and metadata are published to the update feed
...
1. Build the installer: `npm run package`
2. Upload to your update feed:
   - `release/anyBot Setup X.Y.Z.exe`
   - `release/latest.yml`
   - `release/*.blockmap` (optional, for delta updates)
3. Users with `ANYBOT_UPDATE_FEED_URL` configured will see the update in-app
