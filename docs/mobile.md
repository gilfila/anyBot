...
- The mobile composer's microphone control dictates into the draft. **Voice chat** in one-to-one conversations is a continuous loop: it listens, sends, says "On it.", waits for the run however long it takes, speaks a short summary, then listens again. The mic stays off while the bot works and speaks. Group conversations remain text-first.
- Voice follows the desktop's Voice settings. If the desktop has ElevenLabs connected, audio goes through the gateway (`/v1/voice/*`, contributor or operator devices, 30 calls a minute) and the key never leaves the desktop. Otherwise the phone uses its own speech engine: native plugins in the app, browser APIs on the web. See [voice.md](voice.md).
