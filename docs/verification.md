...
- Opt-in live harness smokes (also via `npm run test:live:codex|hermes|cursor`): `tests/live-hermes-smoke.mjs` and `tests/live-cursor-smoke.mjs` create one employee, send one message, and assert a succeeded run with non-empty output. They exit early with a clear SKIP message when the harness is not detected. Cursor requires a model argv; Hermes may use the harness default when model is omitted.
