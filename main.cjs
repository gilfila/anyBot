...
function loadStartupPreference() {
  try {
    return JSON.parse(fs.readFileSync(startupPath(), "utf8")).launchAtLogin === true;
  } catch {
    return false;
  }
}
function applyStartupPreference(enabled) {
  ...
  try {
    ...
    fs.writeFileSync(startupPath(), JSON.stringify({ launchAtLogin }, null, 2));
  } catch {
...
if (!app.requestSingleInstanceLock()) app.quit();
  ...
  app.whenReady().then(() => {
    applyStartupPreference(loadStartupPreference());
    ipcMain.handle("anybot:request", async (event, method, payload) => {
      ...
      if (method === "runtime.startup") {
        ...
        return {
          ...result,
          runtime: { ...result.runtime, launchAtLogin, userDataFallback },
        };
      }
      if (method === "snapshot") {
        await waitForReady();
        const result = await request(method, payload);
        result.runtime = { ...result.runtime, launchAtLogin, userDataFallback };
        return result;
      }
...
function showWindow(rendererSandbox = app.isPackaged) {
  ...
  window.on("close", (event) => {
    if (!quitting) {
      ...
      // Closing the main window should exit the desktop app instead of leaving
      // a hidden coordinator process running after the UI disappears.
      app.quit();
    }
  });
}
