...
**Next:** Phase 4, the knowledge graph plus its tab on the Organization page (0.3.3).
**Shared working tree:** another session (Codex, avatar "Character Studio" redesign) edits `src/constants.js`, `src/lib/avatar-config.js`, `src/lib/bot-avatar-*.js`, `.superdesign/`, and `design/bot-redesign-options.md` in this checkout. Never stage those. Commit with explicit paths, not `git add -A src`.
