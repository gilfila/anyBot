...
| `gemini.jsonl` | Gemini CLI `--output-format stream-json` shapes (`stats.input_tokens` includes `stats.cached`). Not captured: on 2026-09-24 Gemini CLI 0.57.0 on the owner's PC fails at sign-in (`IneligibleTierError`: Gemini Code Assist for individuals no longer supports the CLI). |
